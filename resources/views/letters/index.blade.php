...
<body>
  ...
  <div class="dash-layout">
    ...
    <main class="dash-main">
      ...
      <div class="dash-content">
        ...
        <!-- Letters -->
        <section class="reveal reveal-delay-1" aria-label="Love letters list" data-gsap-reveal>
          @if ($received->isEmpty() && $mine->isEmpty())
          ...
          @else
            @if ($received->isNotEmpty())
              <div class="letter-section">
                <h2 class="letter-section-title"><i class="fas fa-inbox"></i> Received</h2>
                <div class="letter-list">
                  @foreach ($received as $letter)
                    <article class="letter-card {{ $letter->read_at ? '' : 'unread' }}" data-gsap-reveal>
                      ...
                      <div class="letter-card-body">
                        <div class="letter-card-top">
                          ...
                          @if (! $letter->read_at)
                            <span class="letter-new-badge"><i class="fas fa-circle"></i> New</span>
                          @endif
                        </div>
                        <p class="letter-card-excerpt">{{ Str::limit(strip_tags($letter->content), 140) }}</p>
                        <div class="letter-card-meta">
                          <span><i class="fas fa-heart"></i> From: {{ $letter->user->name }}</span>
            ...
            @if ($mine->isNotEmpty())
              <div class="letter-section">
                <h2 class="letter-section-title"><i class="fas fa-feather-alt"></i> My Letters</h2>
                <div class="letter-list">
                  @foreach ($mine as $letter)
                    <article class="letter-card {{ $letter->is_pinned ? 'pinned' : '' }}" data-gsap-reveal>
                      @if ($letter->is_pinned)
                      ...
                      <div class="letter-card-body">
                        ...
                        <div class="letter-card-meta">
                          @if ($letter->receiver)
                            <span><i class="fas fa-paper-plane"></i> To: {{ $letter->receiver->name }}</span>
                          @endif
                          <span><i class="fas fa-calendar"></i> {{ $letter->letter_date->format('M j, Y') }}</span>
                          <span><i class="fas fa-smile"></i> {{ $letter->mood->label() }}</span>
                        </div>
                ...
                @if ($mine->hasPages())
                  <div class="mem-pagination-wrap">
                    {{ $mine->links('vendor.pagination.memorify') }}
                  </div>
                @endif
              </div>
            @endif
  ...
  @vite('resources/js/letters-animations.js')
