...
<head>
  ...
  <title>Write a Love Letter - Memorify</title>
  <meta name="description" content="Write a love letter." />
...
<body>
    ...
    <main class="dash-main">
      ...
      <div class="dash-content">
        ...
        <section class="mem-head reveal" data-gsap-reveal>
          <div>
            <h1 class="mem-head-title">Write a Love Letter</h1>
            <p class="mem-head-sub">Say what your heart feels.</p>
          </div>
        ...
        <section class="letter-form-wrap reveal reveal-delay-1" data-gsap-reveal>
          <form method="POST" action="{{ route('letters.store') }}" class="letter-form" data-submit-feedback>
            @csrf

            <div class="form-group">
              <label for="receiver_id">To</label>
              <select id="receiver_id" name="receiver_id" class="form-control @error('receiver_id') is-invalid @enderror">
                <option value="">Just for me (personal letter)</option>
                @foreach ($partners as $partner)
                  <option value="{{ $partner->id }}" {{ old('receiver_id', $selectedReceiverId) == $partner->id ? 'selected' : '' }}>
                    {{ $partner->name }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="My dearest..." class="form-control @error('title') is-invalid @enderror" />
              @error('title')
                <span class="form-error">{{ $message }}</span>
              @enderror
            </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <select id="mood" name="mood" class="form-control @error('mood') is-invalid @enderror">
                  @foreach (\App\Enums\LoveLetterMood::cases() as $mood)
                    <option value="{{ $mood->value }}" {{ old('mood') === $mood->value ? 'selected' : '' }}>
                      {{ $mood->label() }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="form-group">
                ...
                <input type="date" id="letter_date" name="letter_date" value="{{ old('letter_date', now()->toDateString()) }}" class="form-control @error('letter_date') is-invalid @enderror" />
                @error('letter_date')
                  <span class="form-error">{{ $message }}</span>
                @enderror
              </div>
            ...
            <div class="form-group">
              ...
              <textarea id="content" name="content" class="rte-hidden" hidden>{{ old('content') }}</textarea>
              @error('content')
                <span class="form-error">{{ $message }}</span>
              @enderror
            </div>
            ...
            <div class="letter-form-actions">
              <button type="submit" class="btn btn-primary"><i class="fas fa-feather-alt"></i> Send Letter</button>
              <a href="{{ route('letters.index') }}" class="btn btn-outline">Cancel</a>
            </div>
          </form>
        </section>
  ...
  @vite('resources/js/letters-form-animations.js')
  <script src="{{ assetv('js/main.js') }}"></script>
  <script src="{{ assetv('js/dashboard.js') }}"></script>
  <script src="{{ assetv('js/letters.js') }}"></script>
</body>
</html>
