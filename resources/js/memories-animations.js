...
/**
 * Find the revealable container for the memories grid or empty state.
 */
 ...
 * Transform/opacity only; clears inline styles once done.
 */
function memoriesReveal(isMobile) {
...
createMotionModule('Memories', ({ matchMedia }) => {
  memoriesEntrance();
  ...
  mm.add({ mobile: '(max-width: 680px)', desktop: '(min-width: 681px)' }, (context) => {
    ...
    const toolbar = document.querySelector('.mem-toolbar[data-gsap-reveal]');
    if (toolbar) {
      gsap.fromTo(
        toolbar,
        { opacity: 0, y: isMobile ? 14 : 24 },
        {
          ...
          onComplete: () => gsap.set(toolbar, { clearProps: 'transform,opacity' }),
        }
      );
    }

    memoriesReveal(isMobile);
  });
});
