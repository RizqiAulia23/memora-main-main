...
/**
 * Reveal the main content section once on scroll.
 * Transform/opacity only; clears inline styles once done.
 */
...
createMotionModule('CalendarEvents', ({ matchMedia }) => {
  calendarEventsEntrance();

  const mm = matchMedia();
  mm.add({ mobile: '(max-width: 680px)', desktop: '(min-width: 681px)' }, (context) => {
    eventsReveal(context.conditions.mobile);
  });
});
